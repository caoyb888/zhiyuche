...
const lineColumns: Column<SettlementLine>[] = [
  ...
  { key: 'occurred_at', title: '日期', render: (l) => <span className="text-xs text-slate-600 whitespace-nowrap">{formatDateTime(l.occurred_at)}</span> },
  ...
  { key: 'amount', title: '金额（元）', align: 'right', render: (l) => <span className={clsx('font-mono font-medium', l.kind === 'penalty' ? 'text-red-600' : 'text-slate-800')}>{formatMoney(l.amount)}</span> },
]
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="text-base font-semibold text-slate-800">{s.dept_id ? (s.dept_name ?? '部门') : '企业汇总'}</h4>
          ...
          <div className="mt-0.5 text-xs text-slate-400">结算期 {s.period} · 生成于 {formatDateTime(s.generated_at)}</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-lg bg-brand-50 px-3 py-2 col-span-2 sm:col-span-1">
          <div className="text-[11px] text-brand-700/70">合计（元）</div>
          <div className="mt-0.5 font-mono text-lg font-semibold text-brand-700">{formatMoney(s.total)}</div>
        </div>
        <div className="rounded-lg bg-slate-50 px-3 py-2">
          <div className="text-[11px] text-slate-400">行程费 · {s.trip_count} 次</div>
          <div className="mt-0.5 font-mono text-sm font-medium text-slate-800">{formatMoney(s.trip_cost)}</div>
        </div>
        <div className="rounded-lg bg-slate-50 px-3 py-2">
          <div className="text-[11px] text-slate-400">充电费 · {s.charge_count} 次</div>
          <div className="mt-0.5 font-mono text-sm font-medium text-slate-800">{formatMoney(s.charge_cost)}</div>
        </div>
        <div className="rounded-lg bg-slate-50 px-3 py-2">
          <div className="text-[11px] text-slate-400">罚金</div>
          <div className={clsx('mt-0.5 font-mono text-sm font-medium', s.penalty > 0 ? 'text-red-600' : 'text-slate-800')}>{formatMoney(s.penalty)}</div>
        </div>
      </div>
      ...
      <DescriptionList
        ...
        items={[
          ...
          { label: '预算使用率', value: usage === null ? '—' : <span className={usage >= 100 ? 'text-red-600 font-medium' : undefined}>{usage.toFixed(1)}%</span> },
            ...
            <div key={g.kind}>
              <div className="mb-1 flex items-center justify-between text-xs">
                ...
                <span className="text-slate-500">
                  小计 <span className={clsx('font-mono font-medium', g.kind === 'penalty' ? 'text-red-600' : 'text-slate-800')}>{formatMoney(g.subtotal)}</span>
                </span>
              </div>
