    ...
    return (
      <div className={controlClass(invalid, 'flex h-9 items-center gap-2 px-3')}>
        ...
        <span className="min-w-0 flex-1 truncate">
          ...
          {value.username && <span className="ml-1 text-xs text-slate-400">@{value.username}</span>}
          {value.dept_name && <span className="ml-1 text-xs text-slate-400">· {value.dept_name}</span>}
        </span>
        {!disabled && (
          <button type="button" aria-label="清除持卡人" onClick={() => onChange(null)} className="rounded p-0.5 text-slate-400 hover:text-slate-600">
            <X size={14} />
          </button>
        )}
      </div>
      ...
      {open && keyword.trim() && (
        <div className="absolute z-40 mt-1 w-full rounded-lg border border-slate-200 bg-white shadow-lg">
          {users.isPending && debounced ? (
            ...
            <div className="px-3 py-3 text-xs text-red-500">{errorMessage(users.error)}</div>
          ) : items.length === 0 ? (
            <div className="px-3 py-3 text-center text-xs text-slate-400">{debounced ? '无匹配用户' : '输入关键词搜索'}</div>
          ) : (
              ...
              {items.map((u) => (
                <li key={u.id}>
                  <button
                    ...
                    className={clsx('flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-slate-50')}
                  >
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-50 text-[11px] font-medium text-brand-700">{u.name.slice(0, 1)}</span>
                    <span className="min-w-0 flex-1 truncate">
                      <span className="text-slate-800">{u.name}</span>
                      <span className="ml-1 text-xs text-slate-400">@{u.username}</span>
                    </span>
                    {u.dept_name && <span className="shrink-0 text-xs text-slate-400">{u.dept_name}</span>}
                  </button>
                </li>
              ))}
            </ul>
