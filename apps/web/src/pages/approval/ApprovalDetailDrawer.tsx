...
const STEP_ICON: Record<ApprovalStepAction, { icon: typeof Check; cls: string }> = {
  pending: { icon: Clock, cls: 'bg-amber-100 text-amber-600' },
  approved: { icon: Check, cls: 'bg-emerald-100 text-emerald-600' },
  rejected: { icon: X, cls: 'bg-red-100 text-red-600' },
...
function StepTimeline({ a }: { a: Approval }) {
  ...
  if (steps.length === 0) return <div className="text-xs text-slate-400">暂无审批步骤</div>
      ...
      {steps.map((s: ApprovalStep, i) => {
        ...
        return (
          <li key={s.step_no} className="relative flex gap-3 pb-4">
            {!last && <span className="absolute left-3 top-6 h-[calc(100%-0.5rem)] w-px bg-slate-200" aria-hidden />}
            <span className={clsx('relative z-[1] flex h-6 w-6 shrink-0 items-center justify-center rounded-full', meta.cls, current && 'ring-2 ring-amber-300 ring-offset-1')}>
              <Icon size={13} />
            </span>
            <div className={clsx('min-w-0 flex-1 rounded-lg px-3 py-2', current ? 'bg-amber-50' : 'bg-slate-50')}>
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                <span className="text-xs text-slate-400">第 {s.step_no} 级</span>
                <span className="font-medium text-slate-800">{s.approver.name}</span>
                {s.approver.dept_name && <span className="text-xs text-slate-400">{s.approver.dept_name}</span>}
                <Badge color={STEP_ACTION_BADGE[s.action]}>{current ? '当前步骤' : STEP_ACTION_LABEL[s.action]}</Badge>
              </div>
              {s.remark && <div className="mt-1 text-xs text-slate-600">备注：{s.remark}</div>}
              <div className="mt-1 text-[11px] text-slate-400">{s.acted_at ? formatDateTime(s.acted_at) : current ? '等待处理' : '—'}</div>
            </div>
          </li>
        )
      })}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-mono text-base font-semibold text-slate-800">{a.apply_no}</span>
        ...
        {a.level_required === 2 && <span className="text-xs text-slate-400">需二级审批</span>}
      </div>

      {(a.reject_reason || a.cancel_reason) && (
        <div className={clsx('rounded-xl border p-3 text-sm', a.reject_reason ? 'border-red-100 bg-red-50 text-red-700' : 'border-slate-200 bg-slate-50 text-slate-600')}>
      ...
      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">申请信息</h4>
        <DescriptionList
          columns={2}
          items={[
            {
              ...
              value: (
                <span>
                  {a.applicant.name}
                  {a.dept_name && <span className="ml-1 text-xs text-slate-400">{a.dept_name}</span>}
                </span>
              ),
            },
            ...
            { label: '具体说明', value: a.purpose_detail, span: 2 },
            {
              ...
              value: a.vehicle ? (
                <span className="flex flex-wrap items-center gap-1.5">
                  ...
                  {a.vehicle.model && <span className="text-xs text-slate-400">{a.vehicle.model}</span>}
                  <Badge color={VEHICLE_STATUS_BADGE[a.vehicle.status]}>{VEHICLE_STATUS_LABEL[a.vehicle.status]}</Badge>
                </span>
              ) : (
                <span className="text-slate-400">未指定（由审批人指派）</span>
              ),
            },
      ...
      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">目的地与计划路线</h4>
        <RoutePreview destination={dest} destinationLabel={a.destination || '目的地'} route={a.planned_route ?? null} height={240} />
      </section>

      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">审批流</h4>
        <StepTimeline a={a} />
      </section>

      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">关联行程</h4>
        {a.trip ? (
          <Link to={`/trips?id=${encodeURIComponent(a.trip.id)}`} className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5 text-sm transition-colors hover:border-brand-200 hover:bg-brand-50/40">
            <span className="flex flex-wrap items-center gap-2">
              <span className="font-mono font-medium text-slate-800">{a.trip.trip_no}</span>
              <Badge color={TRIP_STATUS_BADGE[a.trip.status]}>{TRIP_STATUS_LABEL[a.trip.status]}</Badge>
              <span className="text-xs text-slate-500">{formatTimeRange(a.trip.start_at, a.trip.end_at)}</span>
              <span className="text-xs text-slate-500">{formatKm(a.trip.distance_km, 1)}</span>
            </span>
            <ChevronRight size={16} className="shrink-0 text-slate-300" />
          </Link>
        ) : (
          <div className="text-xs text-slate-400">{a.status === 'approved' ? '已批准，等待刷卡取车或手动开始行程' : '暂无关联行程'}</div>
        )}
        {a.trip && a.trip.status === 'cancelled' && a.status === 'approved' && (
          <div className="mt-2 text-xs text-slate-400">上一次行程已作废，申请已回到「已批准」，可重新刷卡取车或手动开始行程</div>
        )}
      </section>

      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">附件</h4>
        {a.attachments && a.attachments.length > 0 ? (
          <ul className="space-y-1.5">
            {a.attachments.map((f, i) => (
              <li key={`${f.url ?? ''}-${i}`}>
                <a href={f.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm text-brand-700 hover:underline">
                  <Paperclip size={13} />
      ...
      {hasActions && (
        <div className="sticky bottom-0 -mx-5 -mb-4 flex flex-wrap items-center justify-end gap-2 border-t border-slate-100 bg-white px-5 py-3">
          ...
          {canCancel && (
            <Button variant="secondary" icon={Undo2} className="text-slate-600" onClick={() => setAction('cancel')}>
              撤销
            </Button>
          )}
          {canApprove && (
