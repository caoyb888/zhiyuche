...
function MeterGrid({ t }: { t: ChargeTransaction }) {
  ...
  const cells: StatCell[] = [
    { label: '桩侧计量', value: formatKwh(t.kwh), sub: typeof t.meter_start === 'number' ? `电表 ${t.meter_start} → ${t.meter_stop ?? '—'} Wh` : undefined, color: 'text-warn-200' },
    ...
    { label: '偏差', value: formatPct(t.deviation_pct), sub: t.deviation_pct === null || t.deviation_pct === undefined ? '无 BMS 数据' : over ? '超过 5%，需人工复核' : '在 5% 阈值内', color: over ? 'text-danger-200' : typeof t.deviation_pct === 'number' ? 'text-ev-200' : undefined, bg: over ? 'bg-danger-500/10' : undefined },
  ]
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {cells.map((c) => (
        <div key={c.label} className={clsx('rounded-xl p-3', c.bg ?? 'bg-surface-3')}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-mono text-base font-semibold text-ink-strong">{t.tx_no}</span>
        <Badge color={CHARGE_STATUS_BADGE[t.status]}>
          {ongoing && <span className="inline-block h-1.5 w-1.5 rounded-full bg-warn-500 pulse-dot" />}
        ...
        {ongoing && <span className="text-xs text-ink-faint">每 10 秒刷新</span>}
      </div>
          ...
          items={[
            {
              ...
              value: (
                <span>
                  {t.pile_name} <span className="font-mono text-xs text-ink-faint">{t.pile_code}</span> · {t.connector_id} 号枪
                </span>
              ),
            },
            ...
            { label: '结束时间', value: t.end_at ? formatDateTime(t.end_at) : <span className="text-warn-200">充电中</span> },
      ...
      <section>
        <h4 className="mb-3 text-sm font-semibold text-ink">归属</h4>
        <DescriptionList
          columns={2}
          items={[
            {
              ...
              value: t.user ? (
                <span>
                  {t.user.name}
                  {(t.dept_name ?? t.user.dept_name) && <span className="ml-1 text-xs text-ink-faint">{t.dept_name ?? t.user.dept_name}</span>}
                </span>
              ) : (
            ...
            },
            {
              ...
              value: t.vehicle ? (
                ...
                <span className="text-warn-200">未绑定车辆</span>
              ),
            },
            { label: '绑定方式', value: bindMethodLabel(t.bind_method) },
            { label: '扣费账户级别', value: attribution ?? (t.status === 'settled' ? '—' : <span className="text-ink-faint">结算后确定</span>) },
          ]}
        />
      </section>

      <section>
        <h4 className="mb-3 text-sm font-semibold text-ink">计量与交叉校验</h4>
        <MeterGrid t={t} />
      </section>
      ...
      <section>
        ...
        <div className="rounded-xl border border-line">
          <div className="grid grid-cols-3 divide-x divide-line-soft text-sm">
            <div className="px-4 py-3">
              <div className="text-xs text-ink-faint">单价</div>
              <div className="mt-0.5 font-medium text-ink-strong">{typeof t.unit_price === 'number' ? `${t.unit_price.toFixed(2)} 元/kWh` : '—'}</div>
            </div>
            <div className="px-4 py-3">
              <div className="text-xs text-ink-faint">费用</div>
              <div className={clsx('mt-0.5 font-semibold', typeof t.cost === 'number' ? 'text-ev-200' : 'text-ink-faint')}>{typeof t.cost === 'number' ? `¥ ${formatMoney(t.cost)}` : t.status === 'settled' ? '¥ 0.00' : t.review_status === 'rejected' ? '不计费' : '待结算'}</div>
            </div>
            <div className="px-4 py-3">
              <div className="text-xs text-ink-faint">扣费流水</div>
              <div className="mt-0.5 font-medium text-ink-strong">
                {typeof t.account_txn_id === 'number' ? (
                  <Link to={`/billing/accounts?txn=${t.account_txn_id}`} className="inline-flex items-center gap-1 text-brand-300 hover:underline">
                    #{t.account_txn_id}
      ...
      <section>
        <div className="mb-2 flex items-center justify-between">
          <h4 className="text-sm font-semibold text-ink">电表曲线</h4>
        ...
        {meter.isPending && !t.meter_values?.length ? (
          <div className="flex h-[220px] items-center justify-center rounded-xl bg-surface-3">
            <Spinner label="加载曲线…" />
          </div>
        ) : (
      ...
      <section>
        <h4 className="mb-3 text-sm font-semibold text-ink">复核信息</h4>
        {t.review_status === 'none' ? (
          <div className="rounded-xl bg-surface-3 px-4 py-3 text-sm text-ink-faint">归属明确且计量偏差在阈值内，无需人工复核</div>
        ) : (
          <DescriptionList
            columns={2}
            items={[
      ...
      {(canReview || canStop) && (
        <div className="sticky bottom-0 -mx-5 -mb-4 flex flex-wrap items-center justify-end gap-2 border-t border-line bg-surface-2 px-5 py-3">
          {canStop && (
            <Button variant="secondary" icon={Square} className="text-danger-200" onClick={() => setStopTarget({ pileId: t.pile_id, pileName: t.pile_name, transaction: t })}>
              远程停止
            </Button>
          )}
          {canReview && (
