...
function StatGrid({ t }: { t: Trip }) {
  const cells: StatCell[] = [
    { label: '里程', value: formatKm(t.distance_km, 1), color: 'text-blue-600' },
    { label: '时长', value: formatMinutes(t.duration_min) },
    { label: '平均速度', value: formatSpeed(t.avg_speed) },
    { label: '最高速度', value: formatSpeed(t.max_speed), color: typeof t.max_speed === 'number' && t.max_speed > 100 ? 'text-red-600' : undefined },
    { label: '耗电', value: formatNumber(t.energy_kwh, 1, 'kWh'), sub: typeof t.energy_per_100km === 'number' ? `百公里 ${t.energy_per_100km.toFixed(1)} kWh` : undefined, color: 'text-amber-600' },
    { label: 'SOC 起止', value: `${formatPercent(t.start_soc)} → ${formatPercent(t.end_soc)}` },
    { label: '急加速 / 急刹', value: `${t.harsh_accel} / ${t.harsh_brake} 次`, color: t.harsh_accel + t.harsh_brake > 8 ? 'text-red-600' : undefined },
    { label: '灯牌', value: ROOF_SIGN_LABEL[t.roof_sign_status] },
    { label: '偏离最大距离', value: t.deviation_flag ? (typeof t.deviation_max_m === 'number' ? (t.deviation_max_m >= 1000 ? `${(t.deviation_max_m / 1000).toFixed(1)} km` : `${Math.round(t.deviation_max_m)} m`) : '有偏离') : '无偏离', color: t.deviation_flag ? 'text-red-600' : 'text-emerald-600' },
  ]
  return (
          ...
          <li key={e.id} className="relative flex gap-3 pb-3">
            ...
            <div className={clsx('min-w-0 flex-1 rounded-lg px-3 py-2', cls.bg)}>
              <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-0.5">
                ...
                <span className="font-mono text-[11px] text-slate-400">{formatDateTime(e.ts)}</span>
              </div>
              <div className="text-xs text-slate-600">{describeEvent(e)}</div>
            </div>
          </li>
        )
      })}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-mono text-base font-semibold text-slate-800">{t.trip_no}</span>
        <Badge color={TRIP_STATUS_BADGE[t.status]}>
          {ongoing && <span className="inline-block h-1.5 w-1.5 rounded-full bg-blue-500 pulse-dot" />}
        ...
        <span className="text-xs text-slate-400">{TRIP_SOURCE_LABEL[t.source]}</span>
      </div>

      <section>
        <div className="mb-2 flex items-center justify-between">
          <h4 className="text-sm font-semibold text-slate-700">轨迹回放</h4>
        ...
        {track.isPending ? (
          <div className="flex h-[380px] items-center justify-center rounded-xl bg-slate-50">
            <Spinner label="加载轨迹…" />
          </div>
        ) : track.isError ? (
      ...
      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">行程数据</h4>
        <StatGrid t={t} />
      </section>

      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">车辆 / 驾驶员 / 申请</h4>
        <DescriptionList
          columns={2}
          items={[
            {
            ...
            },
            {
              ...
              value: t.driver ? (
                <span>
                  {t.driver.name}
                  {t.driver.dept_name && <span className="ml-1 text-xs text-slate-400">{t.driver.dept_name}</span>}
                  {t.card_uid && <span className="ml-1 font-mono text-xs text-slate-400">卡 {t.card_uid}</span>}
                </span>
              ) : (
            ...
            { label: '结束时间', value: t.end_at ? formatDateTime(t.end_at) : <span className="text-blue-600">进行中</span> },
            ...
            { label: '事由', value: t.purpose ?? t.approval?.purpose_detail ?? null },
            {
              label: '关联申请',
              value: t.approval?.id ? (
                <Link to={`/approval?id=${encodeURIComponent(t.approval.id)}`} className="inline-flex items-center gap-1 text-brand-700 hover:underline">
                  <span className="font-mono">{t.approval.apply_no}</span>
                  {t.approval.destination && <span className="text-xs text-slate-500">· {t.approval.destination}</span>}
                  {typeof t.approval.planned_km === 'number' && <span className="text-xs text-slate-400">· 预计 {t.approval.planned_km.toFixed(1)} km</span>}
                  <ChevronRight size={13} />
                </Link>
              ) : (
                <span className="text-slate-400">无（{TRIP_TYPE_LABEL[t.trip_type]}）</span>
              ),
            },
      ...
      <section>
        <h4 className="mb-3 text-sm font-semibold text-slate-700">事件（{events.length}）</h4>
      ...
      {hasActions && (
        <div className="sticky bottom-0 -mx-5 -mb-4 flex flex-wrap items-center justify-end gap-2 border-t border-slate-100 bg-white px-5 py-3">
          <Button variant="secondary" icon={Ban} className="text-red-600" onClick={() => setAction('cancel')}>
            作废
          </Button>
