...
function Summary({ t }: { t: Trip }) {
  return (
    <div className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
      <span className="font-mono text-slate-700">{t.trip_no}</span>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <Summary t={t} />
        <div className="text-xs text-slate-500">结束后按轨迹汇总里程、能耗、速度与急加减速；车辆回到空闲，关联申请置为已完成。里程表 / SOC 留空时取最后一条遥测。</div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            <label htmlFor="end-odo" className="block text-sm font-medium text-slate-700">
              结束里程表（km）
            </label>
          ...
          <div className="space-y-1.5">
            <label htmlFor="end-soc" className="block text-sm font-medium text-slate-700">
              结束 SOC（%）
            </label>
        ...
        <div className="space-y-1.5">
          <label htmlFor="end-remark" className="block text-sm font-medium text-slate-700">
            备注
          </label>
