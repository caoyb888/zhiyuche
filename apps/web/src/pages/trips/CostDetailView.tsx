...
function amountClass(l: CostLine): string {
  if (l.kind === 'penalty') return 'text-red-600'
  if (l.kind === 'cap' || l.amount < 0) return 'text-emerald-600'
  if (l.kind === 'surcharge') return 'text-amber-700'
...
export default function CostDetailView({ cost, detail, billing }: CostDetailViewProps) {
  ...
  if (!isNum(cost)) {
    ...
    return (
      <div className={clsx('flex items-start gap-2 rounded-xl px-4 py-3 text-sm', failed ? 'bg-red-50 text-red-700' : 'bg-slate-50 text-slate-400')}>
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {billing?.billing_status === 'skipped' && <span className="text-xs text-slate-400">该行程不计费（无生效规则或规则不适用）</span>}
          </div>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-100">
      {d && d.lines.length > 0 ? (
                ...
                <tr key={`${l.kind}-${l.item}-${i}`}>
                  <td>
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className={clsx('text-sm', l.kind === 'penalty' ? 'text-red-700' : 'text-slate-700')}>{l.item}</span>
                      {l.kind !== 'base' && (
                    ...
                    {l.note && l.kind !== 'multiplier' && <div className="mt-0.5 text-[11px] text-slate-400">{l.note}</div>}
                  </td>
                  <td className="whitespace-nowrap text-right text-xs text-slate-600">{qtyText(l)}</td>
                  <td className="whitespace-nowrap text-right text-xs text-slate-500">{priceText(l)}</td>
        ...
        <div className="px-4 py-3 text-xs text-slate-400">{d ? '无明细行' : '无计费明细'}</div>
      )}

      {d && (d.multiplier !== 1 || d.cap_applied || d.penalty > 0 || d.surcharge > 0 || d.electricity > 0) && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-slate-50 px-4 py-2 text-[11px] text-slate-500">
          <span>基础 ¥ {formatMoney(d.base)}</span>
          {d.multiplier !== 1 && <span className="text-blue-600">时段系数 ×{d.multiplier}</span>}
          {d.cap_applied && <span className="text-emerald-600">已触发日封顶</span>}
          {d.surcharge > 0 && <span className="text-amber-700">附加 ¥ {formatMoney(d.surcharge)}</span>}
          {d.electricity > 0 && <span className="text-amber-700">电费 ¥ {formatMoney(d.electricity)}</span>}
          {d.penalty > 0 && <span className="text-red-600">罚金 ¥ {formatMoney(d.penalty)}</span>}
        </div>
      )}

      <div className="flex flex-wrap items-center justify-between gap-2 bg-brand-50 px-4 py-3">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-brand-800/80">
          ...
          {typeof txnId === 'number' && (
            <Link to={`/billing/accounts?txn=${txnId}`} className="inline-flex items-center gap-0.5 text-brand-700 hover:underline">
              流水 #{txnId}
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-lg font-bold text-brand-700">¥ {formatMoney(cost)}</span>
        </div>
      </div>
    </div>
  )
