...
const scoreColor = (s: number): string => s >= 85 ? '#10b981' : s >= 70 ? '#f59e0b' : '#ef4444'
...
const aiItemStyle: Record<AiItemStatus, { box: string; label: string; value: string }> = {
  bad:  { box:'border-red-100 bg-red-50',         label:'text-red-600',     value:'text-red-700' },
  warn: { box:'border-amber-100 bg-amber-50',     label:'text-amber-600',   value:'text-amber-700' },
  good: { box:'border-emerald-100 bg-emerald-50', label:'text-emerald-600', value:'text-emerald-700' },
}
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-3 slide-up">
                  ...
                  <div className="col-span-2 md:col-span-4 flex gap-2 mt-1">
                    ...
                    <button type="button" onClick={e => e.stopPropagation()}
                      className="flex items-center gap-1.5 bg-slate-50 text-slate-600 text-xs px-3 py-2 rounded-xl hover:bg-slate-100 border border-slate-200">
