...
import type { Approval, ApprovalPurpose, ApprovalStatus, BadgeClass } from '../types'
import { CheckCircle, XCircle, Clock, ChevronRight, FileText, MapPin, Calendar, User, type LucideIcon } from 'lucide-react'

const statusMap: Record<ApprovalStatus, { cls: BadgeClass; icon: LucideIcon }> = {
  '待审批': { cls:'badge-amber', icon:Clock },
  '已通过': { cls:'badge-green', icon:CheckCircle },
  '已驳回': { cls:'badge-red',   icon:XCircle },
}

type ApprovalFilter = '全部' | ApprovalStatus
const tabs: ApprovalFilter[] = ['全部','待审批','已通过','已驳回']

/** 发起申请表单字段（演示用，未接后端） */
type FormField =
  | { label: string; type: 'select'; opts: ApprovalPurpose[] }
  | { label: string; type: 'textarea' }
  | { label: string; type: 'text' | 'datetime-local'; ph?: string }
...
function ApprovalDetail({ item, onClose, onApprove, onReject }: ApprovalDetailProps) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl slide-up" onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center font-semibold text-brand-700 text-sm">{item.applicant.charAt(0)}</div>
            <div>
          ...
          {item.status !== '已驳回' && (
            <div className="bg-brand-900 rounded-xl p-4">
              ...
              <div className="flex items-center justify-center">
                <button type="button" onClick={() => setSignOn(p=>!p)}
          ...
          {item.status === '待审批' && !showReject && (
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setShowReject(true)} className="flex-1 py-2.5 border border-red-200 text-red-600 rounded-xl text-sm font-medium hover:bg-red-50 transition-colors">驳回</button>
              <button type="button" onClick={onApprove} className="flex-1 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700 transition-colors">审批通过</button>
            </div>
          )}
          ...
          {showReject && (
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowReject(false)} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-sm hover:bg-slate-50">取消</button>
                <button type="button" onClick={() => onReject(rejectNote)} className="flex-1 py-2.5 bg-red-500 text-white rounded-xl text-sm font-medium hover:bg-red-600">确认驳回</button>
              </div>
            </div>
          )}
        </div>
...
export default function ApprovalPage() {
  ...
  const [filter, setFilter] = useState<ApprovalFilter>('全部')
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl slide-up" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-600 hover:bg-slate-50">取消</button>
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700">提交申请</button>
              </div>
            </div>
          </div>
        </div>
