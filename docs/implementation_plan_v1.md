...
**完成情况（2026-09-09）**：已交付。后端：可编程计费引擎（里程/时长/日封顶/时段系数/低电附加/电费/四类罚金/归属级别，含方案算例单测）、规则 CRUD 与模拟计算、三级账户（企业→部门→员工）与流水、行程结束自动扣费（幂等、透支通知）、充电计价扣费、月度结算生成/确认/xlsx 导出；OCPP 1.6J 服务端（Boot/Heartbeat/Status/Authorize/Start/Meter/Stop、RemoteStart/Stop，与 api 同进程独立监听 :20081）、充电事务与电表曲线、车辆归属（远程预置/桩位/最近行程/刷卡）、桩侧计量与 BMS 估算交叉校验（偏差 >5% 进复核）、复核与远程启停；模拟器新增 OCPP 充电桩客户端与低电量车辆。前端：计费规则编辑器与模拟面板、账户树与充值/划拨/调整、月度结算与打印视图、我的账户、充电管理（桩状态/记录/待复核）、行程费用明细。验证：后端脚本 `verify-be-c.sh`（181 项）、`verify-be-d.sh`（79 项）、OCPP 集成测试 12 子用例；主库联调模拟器 200 秒：21 次行程自动计费、2 次充电（1 次自动结算、1 次进复核后通过计费）、结算生成/确认/导出/重算拦截全部正确。未做：PDF 由浏览器打印视图代替；支付渠道（微信/支付宝）充值仍为线下登记；多实例部署时 OCPP 连接注册表需改为共享存储。
